...
export function createTextFieldState(
    partialInitialState: Partial<TextFieldState> = {},
) {
...
export function createTextFieldReducer(
    initialState: TextFieldState,
): FormElementReducer<TextFieldState> {
    return function (
        state: TextFieldState = initialState,
        ...
        if (
            event.type === TextFieldEventTypes.TEXT_FIELD_STATE_WAS_CHANGED
      && event.payload.id === state.id
        ) {
            return {
                ...state,
